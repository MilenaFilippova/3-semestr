...
//конструктор корирования массива
...
void Array::insert(int elem, int index)
{
 if (index < 0 || index > size)
	throw ArrayException();
 if (size==capacity)
    increaseCapacity(size+1);	 // Закрытая функция, увеличивающая предельный размер 

 for (int j=size-1; j>=index; j--) // Если index==size, этот цикл не выполнится ни разу
	ptr[j+1]=ptr[j];
...
ofstream& operator <<(ostream& out,const Array& arr)
